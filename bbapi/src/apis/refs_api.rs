...
pub async fn repositories_workspace_repo_slug_refs_tags_post(configuration: &configuration::Configuration, repo_slug: &str, workspace: &str, tag: models::Tag) -> Result<models::Tag, Error<RepositoriesWorkspaceRepoSlugRefsTagsPostError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_repo_slug = repo_slug;
    let p_path_workspace = workspace;
    let p_body_tag = tag;
