...
/// struct for typed errors of method [`user_workspaces_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum UserWorkspacesGetError {
    Status400(models::Error),
    Status401(models::Error),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`user_workspaces_workspace_permission_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum UserWorkspacesWorkspacePermissionGetError {
    Status401(models::Error),
...
pub async fn user_workspaces_get(configuration: &configuration::Configuration, sort: Option<&str>, administrator: Option<bool>) -> Result<models::PaginatedWorkspaceAccess, Error<UserWorkspacesGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_sort = sort;
    let p_query_administrator = administrator;

    let uri_str = format!("{}/user/workspaces", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);

    if let Some(ref param_value) = p_query_sort {
        req_builder = req_builder.query(&[("sort", &param_value.to_string())]);
    }
    if let Some(ref param_value) = p_query_administrator {
        req_builder = req_builder.query(&[("administrator", &param_value.to_string())]);
    }
    if let Some(ref user_agent) = configuration.user_agent {
        req_builder = req_builder.header(reqwest::header::USER_AGENT, user_agent.clone());
    }
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::PaginatedWorkspaceAccess`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::PaginatedWorkspaceAccess`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<UserWorkspacesGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn user_workspaces_workspace_permission_get(configuration: &configuration::Configuration, workspace: &str) -> Result<models::WorkspaceMembership, Error<UserWorkspacesWorkspacePermissionGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_workspace = workspace;

    let uri_str = format!("{}/user/workspaces/{workspace}/permission", configuration.base_path, workspace=crate::apis::urlencode(p_path_workspace));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<UserWorkspacesWorkspacePermissionGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
